...
static GLuint gl_compile_shader(GLenum type, std::string const &source) {
    ...
    if (compile_status != GL_TRUE) {
        ...
        GLsizei length = 0;
        glGetShaderInfoLog(shader, GLint(info_log.size()), &length,
                           &info_log[0]);
        std::cerr << "Info log: "
...
GLuint gl_compile_program(std::string const &vertex_shader_source,
                          std::string const &fragment_shader_source) {
    GLuint vertex_shader =
        gl_compile_shader(GL_VERTEX_SHADER, vertex_shader_source);
    GLuint fragment_shader =
        gl_compile_shader(GL_FRAGMENT_SHADER, fragment_shader_source);
    ...
    if (link_status != GL_TRUE) {
        ...
        std::vector<GLchar> info_log(info_log_length, 0);
        GLsizei length = 0;
        glGetProgramInfoLog(program, GLint(info_log.size()), &length,
                            &info_log[0]);
        std::cerr << "Info log: "
                  << std::string(info_log.begin(), info_log.begin() + length);
