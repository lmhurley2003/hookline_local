...
template <typename T>
struct Ramp {
    // This variadic template passes the arguments to the constructor of
    // 'value':
...
struct PlayingSample {
    ...
    //  value will change over 'ramp' seconds to avoid creating audible
    //  artifacts:
    void set_volume(float new_volume, float ramp = 1.0f / 60.0f);
    // set the panning of a sample (use only on samples in "2D" mode; no effect
    // on "3D" samples):
    void set_pan(float new_pan, float ramp = 1.0f / 60.0f);
    // set the position of a sample (use only on samples in "3D" mode; no effect
    // on "2D" samples):
    ...
    //'stop' will fade sample out over 'ramp' seconds and then remove it from
    // the active samples:
    void stop(float ramp = 1.0f / 60.0f);

    // internals:
    // NOTE: PlayingSample is used in a separate thread; so setting these values
    // directly
    //  may result in bad results. Instead, use the functions above, which
    //  perform locking!
    ...
    bool stopped = false;   // was playback stopped (either by running out of
                            // sample, or by stop())?
    ...
    Ramp<glm::vec3> position =
        Ramp<glm::vec3>(std::numeric_limits<float>::quiet_NaN());
    ...
    PlayingSample(Sample const &sample_, float volume_,
                  glm::vec3 const &position_, float half_volume_radius_,
                  bool loop_)
        : data(sample_.data),
          loop(loop_),
          volume(volume_),
          position(position_),
          half_volume_radius(half_volume_radius_) {}
};

// ------- global functions -------

void init();  // call Sound::init() from main.cpp before using any member
              // functions

void shutdown();  // call Sound::shutdown() from main.cpp to gracefully(-ish)
                  // exit

// Call 'Sound::play' to play a sample once.
//   if you hang on to the return value, you can change the panning, volume, or
//   stop playback early.
std::shared_ptr<PlayingSample> play(
    Sample const &sample, float volume = 1.0f,
    float pan = 0.0f  //-1.0f == hard left, 1.0f == hard right
);
// The play_3D version will play a sample in '3D' mode (that is, panning
// determined by listener position):
std::shared_ptr<PlayingSample> play_3D(
    Sample const &sample, float volume, glm::vec3 const &position,
...
// The loop_3D version will loop a sample in '3D' mode (that is, panning
// determined by listener position):
std::shared_ptr<PlayingSample> loop_3D(
    Sample const &sample, float volume, glm::vec3 const &position,
    float half_volume_radius = std::numeric_limits<float>::infinity());

// Listener controls the panning of "3D" samples (ones played using the
// "position" version of the play functions):
struct Listener {
    void set_position_right(glm::vec3 const &new_position,
                            glm::vec3 const &new_right,
                            float ramp = 1.0f / 60.0f);

    // internals:
    Ramp<glm::vec3> position = Ramp<glm::vec3>(0.0f);  // listener's location
    Ramp<glm::vec3> right = Ramp<glm::vec3>(
        1.0f, 0.0f, 0.0f);  // unit vector pointing to listener's right
};
...
//  the set_*/stop/play/... functions already use these helpers, so you
//  shouldn't need to call them unless your code is modifying values directly:
void lock();
void unlock();
