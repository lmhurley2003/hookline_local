...
Sound::Sample::Sample(std::string const &filename) {
    if (filename.size() >= 4 &&
        ...
        throw std::runtime_error("Sample '" + filename +
                                 "' doesn't end in either \".png\" or "
                                 "\".opus\" -- unsure how to load.");
    }
}
...
std::shared_ptr<Sound::PlayingSample> Sound::play(Sample const &sample,
                                                  float play_volume,
                                                  float pan) {
...
std::shared_ptr<Sound::PlayingSample> Sound::loop(Sample const &sample,
                                                  float play_volume,
                                                  float pan) {
    std::shared_ptr<Sound::PlayingSample> playing_sample =
...
void step_direction_ramp(Sound::Ramp<glm::vec3> &ramp) {
    if (ramp.ramp < RAMP_STEP) {
        ...
        if (norm == glm::vec3(0.0f)) {
            if (ramp.target.x <= ramp.target.y &&
                ramp.target.x <= ramp.target.z) {
        ...
        float angle = std::acos(
            glm::clamp(glm::dot(ramp.value, ramp.target), -1.0f, 1.0f));
    ...
    for (auto si = playing_samples.begin(); si != playing_samples.end();
        ...
        if (!(playing_sample.pan.value == playing_sample.pan.value)) {
            ...
            compute_pan_from_listener_and_position(
                ...
                playing_sample.half_volume_radius.value, &start_pan.l,
                &start_pan.r);
            ...
            // 2D panning
            compute_pan_weights(playing_sample.pan.value, &start_pan.l,
                                &start_pan.r);
        ...
        if (!(playing_sample.pan.value == playing_sample.pan.value)) {
            ...
            compute_pan_from_listener_and_position(
                ...
                playing_sample.half_volume_radius.value, &end_pan.l,
                &end_pan.r);
        } else {
            // 2D panning
            compute_pan_weights(playing_sample.pan.value, &end_pan.l,
                                &end_pan.r);
        }
        ...
        // figure out a step to add at each sample so that pan will move
