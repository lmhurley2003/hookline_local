...
void ProjectileSystem::update(float dt, entt::registry &registry, entt::entity player) {
    ...
    if(timer_c >= 4) {
        timer_c = 0;
        spawn_chasing_projectile(glm::vec2(getRandomFloat(-1.0f, 1.0f), 1.0f), registry);
    }
    timer_s += dt;
    if(timer_s >= 3) {
        timer_s = 0;
        ...
        spawn_spray_projectile(glm::vec2(getRandomFloat(-3.0, 3.0f), height_), registry);
    }
...
void ProjectileSystem::spawn_horizontal_projectile(bool from_left, float height, entt::registry &registry) {
    ...
    glm::vec2 position = from_left ? glm::vec2{-3.0f, height} : glm::vec2{3.0f, height};
