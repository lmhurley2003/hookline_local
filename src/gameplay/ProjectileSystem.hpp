...
public:
    ...
    void spawn_horizontal_projectile(bool from_left, float height, entt::registry &registry);
...
private:
    bool check_collision(const glm::vec2 &projectile_pos, const glm::vec2 &player_pos);
    float timer_h = 0.0f;
    float timer_c = 0.0f;
    float timer_s = 0.0f;
};